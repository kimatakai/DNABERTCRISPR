...
model_save_dir_path = f'data/saved_weights/transformer'
model_save_path = model_save_dir_path + f'/transformer_{task}_{fold}.weights.h5'
result_save_dir_path = f'data/result/transformer/'
result_save_path = result_save_dir_path + f'result_transformer_{task}_{fold}.json'
result_mean_save_path = result_save_dir_path + f'result_mean_transformer_{task}_{fold}.json'
...
def test_clf(encodings, labels):
    model_class = transformer_module.clf_class(inputshape=encodings[0].shape)
    result = model_class.test(encodings, labels, model_save_path)
    return result

def test_regr(encodings, reads):
    model_class = transformer_module.regr_class(inputshape=encodings[0].shape)
...
def main():
    ...
    for name in test_list:
        print(f'!!!!!!!!!! {name} !!!!!!!!!!')
        listdata = transformer_module.load_tsv(name)
        encodings = transformer_module.return_encoding(listdata)
        labels = transformer_module.return_labels(listdata)
        reads = transformer_module.return_reads(listdata)
