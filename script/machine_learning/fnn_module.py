...
class clf_class:
    ...
    def build_model(self):
        with tf.device(self.device):
            # model definition
            input_layer = Input(shape=(self.inputlen, ))
            dense1      = Dense(256, kernel_regularizer=l2(1e-5), name="dense1")(input_layer)
            dense1      = BatchNormalization()(dense1)
            dense1      = Dropout(0.5)(dense1)
            dense2      = Dense(128, kernel_regularizer=l2(1e-5), name="dense2")(dense1)
            dense2      = BatchNormalization()(dense2)
            dense2      = Dropout(0.5)(dense2)
            dense3      = Dense(64, kernel_regularizer=l2(1e-5), name="dense3")(dense2)
            dense3      = BatchNormalization()(dense3)
            dense3      = Dropout(0.5)(dense3)
            dense4      = Dense(32, kernel_regularizer=l2(1e-5), name="dense4")(dense3)
    ...
    def train(self, input, labels, save_path):
        print(f'Start training FNN for classification')
        with tf.device(self.device):
            print(self.device)
            print(input.shape)
    ...
    def test(self, input, true_labels, save_path):
        print(f'Test FNN for classification')
        with tf.device(self.device):
            model = self.build_model()
...
class regr_class:
    ...
    def build_model(self):
        with tf.device(self.device):
            ...
            dense4      = BatchNormalization()(dense4)
            dense4      = Dropout(0.5)(dense4)
            output      = Dense(1, activation="linear", name="output")(dense4)
            model       = Model(inputs=[input_layer], outputs=[output])
            # compile and train the model
    ...
    def train(self, input, labels, reads, save_path):
        print(f'Start training FNN for regression')
        with tf.device(self.device):
    ...
    def test(self, input, reads_true, save_path):
        print(f'Test FNN for regression')
        with tf.device(self.device):
            model = self.build_model()
            model.load_weights(save_path)
